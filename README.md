...
<body>
  ...
  <script>
    ...
    const STORAGE_KEY = "bierCounterDeluxeV3";
    ...
    function getCombinedTodayForProfile(name) {
      return state.daily.counts[name].beer + state.daily.counts[name].drinks;
    }
    ...
    function updateStatus() {
      ...
      if (totalToday >= 10) {
        statusTextEl.textContent = `${name} ist heute stabil unterwegs. 🍻`;
      } else if (totalToday >= 5) {
        statusTextEl.textContent = `Läuft bei ${name}: ${beerToday} Bier, ${drinksToday} Drinks`;
      } else if (totalToday === 0) {
        statusTextEl.textContent = "Noch nüchtern heute.";
      } else {
        statusTextEl.textContent = `${name}: ${beerToday} Bier · ${drinksToday} Drinks`;
      }
    }
      ...
      sorted.forEach((name, index) => {
        ...
        item.innerHTML = `
          ...
          <div class="leaderboard-sub">
            🍺 Bier: ${state.allTime[name].beer} gesamt / ${state.daily.counts[name].beer} heute
            <br>
            🍹 Drinks: ${state.allTime[name].drinks} gesamt / ${state.daily.counts[name].drinks} heute
          </div>
        `;
      ...
      state.history.forEach(entry => {
        ...
        item.innerHTML = `
          ...
          <div class="history-total">
            ...
            🍺 Bier: <strong>${entry.totalBeer}</strong> · 🍹 Drinks: <strong>${entry.totalDrinks}</strong>
          </div>
